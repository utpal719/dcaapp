...
def dodca(df):
    ...
    for x,y,i,j in zip([0,brkpoint1,brkpoint2],[brkpoint1,brkpoint2,len(df)],eqns,range(3)):
        ...
        master['decline'].append(df['tmonth'][x:y].values.tolist()) #.to_json(orient='values')
        tdf = pd.Series(equations[i](df['tmonth'][x:y],params[j][0],params[j][1]))
        master['eqn'].append(tdf.tolist()) #.to_json(orient='values')
        master['color'].append(graphcolor[i+1])
    ...
    try:
        master['datapoints'] = {
            ...
            'y': df['prod'].values.tolist() #.to_json(orient='records')
        }
    except:
